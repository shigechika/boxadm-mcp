...
の hit のうち、**ローカル部が同じで別ドメイン**のものだけを `near_misses`
（id / name / login だけに削ったもの）へ隔離する。これがエイリアス／重複アカウントの
兆候として見る価値のある唯一の種類だからで、それ以外の前方一致は
`other_prefix_hits` に件数だけ数え、識別情報は返さない。メールアドレスの形をして
いない検索語はリクエスト前に拒否する（`filter_term` に最小長は無く、1文字でも
実在アカウントが1ページ返ってしまうため）。隔離した結果には、
`note` にその旨を明記する。
...
| `near_misses` | **ローカル部が同じで別ドメイン**のアカウント。エイリアス／重複の兆候で、問い合わせた本人では決してない。上限あり |
| `other_prefix_hits` | それ以外の前方一致の**件数のみ**。別人なので識別情報は返さない |
