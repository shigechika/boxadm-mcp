#!/usr/bin/env python3
"""Exercise every registered tool against the live Box enterprise and report what is broken.
...
Read-only by construction: every tool here reads, and the report prints only
tool names, statuses and row counts — never payloads, which would otherwise
carry account names and file names into logs.
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = await run_probes(
            ...
            # Personal data must not reach the report: Box errors quote the
            # account or item they were asked about, and these tools deal in
            # logins and file names throughout. --traceback still shows the
            # full text on the operator's own terminal when debugging.
            redact_details=True,
        )
    finally:
...
def main() -> None:
    ...
    # Serial by default: Box rate-limits per enterprise, and these tools
    # already page and scan internally — a burst of them is a worse neighbour
    # than a slow smoke test.
